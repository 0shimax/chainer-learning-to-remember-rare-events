...
import chainer
from func_conv_lstm import lstm, _extract_gates
...
class LSTMBase(link.Chain):
    ...
    def __init__(self, in_size, out_size, kernel, stride, pad,
                 lateral_init=None, upward_init=None,
                 bias_init=0, forget_bias_init=0):
        super().__init__(
            upward=L.Convolution2D(in_size, 4*out_size, kernel,
                                    stride=stride, pad=pad, initialW=0),
            lateral=L.Convolution2D(out_size, 4*out_size, kernel,
                                    stride=1, pad=pad, initialW=0, nobias=True),
        )
    ...
    def __call__(self, c, h, x):
        ...
        if c is None:
            ...
            c = variable.Variable(
                ...
                volatile='auto')
        return lstm(c, lstm_in)


class ConvLSTM(LSTMBase):
    ...
    def __init__(self, in_size, out_size, kernel, stride, pad, **kwargs):
        super().__init__(in_size, out_size, kernel, stride, pad, **kwargs)
        self.reset_state()
    ...
    def __call__(self, x):
        ...
        if self.c is None:
            ...
            self.c = variable.Variable(
                xp.zeros((batch, self.state_size, h, w), dtype=x.dtype),
                volatile='auto')
