...
class SqueezeNetExternalLongTermMemory(chainer.Chain):
    version = '0.1.0'
    alpha = 0.1
    def __init__(self, n_class, in_ch, update_weight=0.5, vec_size=4096):
        super().__init__(
            ...
            conv9=L.Convolution2D(512*21, vec_size, 1, pad=0),  # *21
            conv_infer=L.Convolution2D(vec_size, n_class, 1, pad=0),
    ...
    def clear(self):
        ...
        if self.memory is None:
            self.memory = chainer.Variable(self.apply_memory.memory.data, volatile=True)
            self.memory_value = chainer.Variable(self.apply_memory.memory_value.data, volatile=True)
            self.memory_age = chainer.Variable(self.apply_memory.memory_age.data, volatile=True)
    ...
    def __call__(self, x, t=None):
        ...
        self.memory = chainer.Variable( \
            self.apply_memory.memory.data, volatile='AUTO')
        self.memory_value = chainer.Variable( \
            self.apply_memory.memory_value.data, volatile='AUTO')
        self.memory_age = chainer.Variable( \
            self.apply_memory.memory_age.data, volatile='AUTO')

        all_similarities = F.matmul( \
            normed_query, self.memory, transb=True)  # (n_batch, memory_size)
        all_similarities.volatile = 'AUTO'
        ...
        if self.train:
            hint_pool_idxs.volatile = 'AUTO'
            normed_query.volatile = 'AUTO'
            with chainer.no_backprop_mode():
